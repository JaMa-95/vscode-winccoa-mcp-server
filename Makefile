...
# Variables
EXTENSION_NAME := winccoa-mcp-server
VERSION := $(shell node -p "require('./package.json').version")
BIN_DIR := bin
EXT_PUBLISHER := RichardJanisch
EXT_NAME := winccoa-mcp-server
...
ifeq ($(OS),Windows_NT)
	@-$(RMDIR) dist 2>nul || echo "" >nul
	@-$(RMDIR) out 2>nul || echo "" >nul
	@-$(RMDIR) bin 2>nul || echo "" >nul
	@-$(RMDIR) node_modules 2>nul || echo "" >nul
else
