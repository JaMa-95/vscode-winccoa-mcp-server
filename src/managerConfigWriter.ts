...
export class ManagerConfigWriter {
    ...
    static async getNextFreeManagerNumber(projectPath: string): Promise<number> {
        ...
        ExtensionOutputChannel.debug(`ManagerConfigWriter: Next free manager number: ${nextNumber}`);
        return nextNumber;
    }
    ...
    static async addManager(projectPath: string, manager: ManagerEntry): Promise<boolean> {
        ...
        try {
            ...
            const managerLine = 
                `${manager.component.padEnd(16)} | ${startMode} |${secKill} |${restartCount} |${resetMin} |${finalOptions}`;
            ...
            ExtensionOutputChannel.info(`ManagerConfigWriter: Successfully added manager: ${manager.component}`);
            ExtensionOutputChannel.debug(`ManagerConfigWriter: Entry: ${managerLine}`);
            
            return true;
            
        } catch (error: any) {
            ExtensionOutputChannel.error(`ManagerConfigWriter: Failed to add manager: ${error.message}`);
            throw error;
        }
    }
